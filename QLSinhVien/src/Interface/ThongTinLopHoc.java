...
public class ThongTinLopHoc extends javax.swing.JFrame {
    ...
    private int chucNangDaChon;

    public ThongTinLopHoc() {
        this.chucNangDaChon = ChucNang.NONE;
        initComponents();
        dfTableModel = tblDSLop.getModel();
    }
//Phương thức xử lý sự kiện khi lựa chọn ở bảng thay đổi
    ...
    void TblDSLop_SelectionChanged() {
        ...
        if (row >= 0) {
            String msLop = (String) getDfTableModel().getValueAt(row, 0);
            String tenLop = (String) getDfTableModel().getValueAt(row, 1);
            String gvcn = (String) getDfTableModel().getValueAt(row, 2);
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {
        if (getChucNangDaChon() == ChucNang.NONE) {
    ...
    private void btnSaveActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        String tenLop = txtTenLop.getText().trim();
        String gvcn = txtGVCN.getText().trim();
        if (getChucNangDaChon() == ChucNang.ADD) {
            if (CheckInput() == false) {
                return;
            }
        ...
        if (getChucNangDaChon() == ChucNang.UPDATE) {
            if (CheckInput() == false) {
                return;
            }
    ...
    /**
     * @return the dfTableModel
     */
    public DefaultTableModel getDfTableModel() {
        return dfTableModel;
    }

    /**
     * @param dfTableModel the dfTableModel to set
     */
    public void setDfTableModel(DefaultTableModel dfTableModel) {
        this.dfTableModel = dfTableModel;
    }

    /**
     * @return the chucNangDaChon
     */
    public int getChucNangDaChon() {
        return chucNangDaChon;
    }

    /**
     * @param chucNangDaChon the chucNangDaChon to set
     */
    public void setChucNangDaChon(int chucNangDaChon) {
        this.chucNangDaChon = chucNangDaChon;
    }
